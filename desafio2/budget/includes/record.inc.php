...
try {
    ...
    $categoryId = $_POST['categoryId'] ?? ''; // Cambia 'categoriesId' a 'categoryId'
    $typeId = $_POST['typeId'] ?? 1; // Usa el valor enviado por el formulario
    $fecha = $_POST['fecha'] ?? ''; // Cambia 'eventDate' a 'fecha'
    $amount = $_POST['amount'] ?? '';
    ...
    if (empty($categoryId) || empty($fecha) || empty($amount)) {
        ...
        header("Location: ../pages/income.php"); // Redirige a income.php
        exit();
    }
    ...
    if (isset($_FILES['factura']) && $_FILES['factura']['error'] === UPLOAD_ERR_OK) { // Cambia 'receiptsPic' a 'factura'
    ...
    $stmt->execute([
        ...
        ':description' => $description !== '' ? $description : null, // Convertir cadena vacía a NULL
        ':fecha' => $fecha,
        ':amount' => (float)$amount,
        ':factura' => $factura
    ]);
